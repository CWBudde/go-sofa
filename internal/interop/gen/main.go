...
type expectation struct {
	...
	Dimensions map[string]int     `json:"dimensions"` // netCDF dimensions and lengths
...
func positionDatasets(f *sofa.File) map[string]dataset {
	// dim is the first axis: the primary dimension when the count matches
	// it, otherwise the singleton I (as sofa.Save writes them).
	flat := func(vs []sofa.Vector3, primary string, size int) dataset {
		dim := primary
		if len(vs) != size {
			dim = "I"
		}
		d := dataset{Shape: []int{len(vs), 3}, Dims: []string{dim, "C"}}
		for _, v := range vs {
	...
	return map[string]dataset{
		"ListenerPosition": flat(f.ListenerPositions, "M", f.M),
		"ReceiverPosition": flat(f.ReceiverPositions, "R", f.R),
		"SourcePosition":   flat(f.SourcePositions, "M", f.M),
		"EmitterPosition":  flat(f.EmitterPositions, "E", f.E),
		"ListenerUp":       flat([]sofa.Vector3{f.ListenerUp}, "I", 1),
		"ListenerView":     flat([]sofa.Vector3{f.ListenerView}, "I", 1),
	}
}

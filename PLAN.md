...
go-hdf5 v0.16.0). R5 and R6 are done; R7 is done except R7c (blocked on go-hdf5 reader/writer entry points) and R7f (API decision); R8 is done; R9 is done except R9d (release-tag decision). Phase C is done, Phase D except D2
...
### Phase C — Streaming / partial reads ✅ done (2026-09-26)
...
- [x] **C1. Upstream capability check.** Inspect the go-hdf5 API for
  ...
  - (2026-09-26) — go-hdf5 v0.16.1 has `Dataset.ReadSlice(start, count
[]uint64) (interface{}, error)` (returns `[]float64` for float and
    integer data) and `Dataset.ReadHyperslab(*HyperslabSelection)` with
    stride/block, for compact, contiguous and chunked (B-tree v1, filtered)
    layouts. go-sofa uses only `ReadSlice` with `[m:m+k, 0:…, 0:…]`
    selections that span every trailing axis in full: on contiguous data
    (what `Save` writes, always k = 1) that is the linear
    `readContiguousOptimized` path and avoids the E5 bug for selections
    starting inside a row. On chunked data every call re-reads the object
    ...
    `Dataset.Info()`, see E4) and caches it (see E6).
- [x] **C2. Lazy `File` mode.** Add
  ...
  - (2026-09-26) — `OpenLazy` shares `open` with `Open`, resolves the audio
    ...
    `TestOpenLazyFailureReleasesHandle` count descriptors.
- [x] **C3. Per-measurement reader.** Implement
  ...
  - (2026-09-26) — `ReadMeasurement` (FIR), `ReadTFMeasurement` (TF, re/im
    ...
    Mesh2HRTF_HRTF_FourPointHorPlane (all chunked + deflate), plus the
    optional FreeFieldHRTF/HRSOS files when present.
...
- [x] **C5. Benchmark.** `go test -bench BenchmarkStreamVs Eager` over
  ...
  - (2026-09-26) — `BenchmarkStreamVsEager` (eager, stream,
    stream-one) in `sofa_largefiles_test.go`; the file is written once per
    test binary and removed in `TestMain`. Results under
    [Performance baseline](#performance-baseline).

### Phase D — Extended testing & cross-validation

- [x] **D1. Large-file build tag.** Introduce `//go:build largefiles`
  ...
  - (2026-09-26) — a 6400 × 2 × 1024 FIR file (104,857,600 B of
    `Data.IR`) checked by `TestLargeFileSize`,
    `TestLargeFileStreamMatchesEager` and `TestLargeFileLazyHeap` (live
    heap grows by ~3 MB while streaming). The large suite adds nothing to
    the default run; the new default streaming tests add under 1 s,
    mostly opening the reference fixtures.
...
- [x] **D3. Benchmarks for write/read.** Add `BenchmarkWriteLarge`
  ...
  - Acceptance: benchmark numbers (ns/op, MB/s) recorded in PLAN.md
    ...
    are visible.
  - (2026-09-26) — see [Performance baseline](#performance-baseline).
- [x] **D4. CI wiring.** Add a separate GitHub Actions job that runs
      `-tags largefiles -short=false` weekly (cron) only.
  - Acceptance: `.github/workflows/largefiles.yml` exists and is
    green on first run.
  - (2026-09-26) — Mondays 04:17 UTC plus `workflow_dispatch`; runs
    `go test -tags largefiles -run . -bench . -benchtime 1x ./...` after
    fetching the CI fixtures (same pinned actions as test-unit). Green
    locally; first CI run pending.
...
- [ ] **E5. `Dataset.ReadSlice` returns zeros for 3-D+ contiguous
  ...
  - (2026-09-26) — Phase C's per-measurement reads avoid it the same way
    (selections span every trailing axis in full).
- [ ] **E6. Per-call overhead of `Dataset.ReadSlice`.** Found in C1
      ...
      go-sofa works around the chunk part by reading and caching a whole
      chunk row along M in `lazyVariable` (`sofa_stream.go`).
  - Acceptance: go-hdf5 caches the parsed header/layout and chunk index

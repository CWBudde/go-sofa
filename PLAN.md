...
- **R3 Tests on fresh clone:** `just fetch-testdata` + `PROVENANCE.md`,
  ...
  - (2026-09-26) — CI now covers the conventions whose sofacoustics.org
    files it cannot fetch: `testdata/sofar/` (committed, MIT, generated by
    `scripts/make_sofar_fixtures.py` with sofar 1.3.0 / netCDF-C 4.9.3)
    has GeneralTF 2.0, GeneralTF-E 1.0, FreeFieldHRTF 1.0 (plain and SH
...
- [x] **R7c.** `io.ReaderAt`/`io.Writer` entry points (`Read(r)`,
  ...
  - (2026-09-26) — partial: blocked upstream. go-hdf5 v0.16.1 only has
    ...
    entry points here.
  - (2026-09-26) — done on go-hdf5 `93c0ac1`
    ([CWBudde/go-hdf5#5](https://github.com/CWBudde/go-hdf5/pull/5):
    `OpenReader`, `CreateForWriteTo`). `OpenReader(r, size)` and
    `OpenLazyReader(r, size)` share `read` with `Open`/`OpenLazy`;
    `(*File).WriteTo(w)` shares `writeHDF5` with `Save` (which keeps its
    temp file + rename), writes byte-identical output and nothing at all on
    a validation or encoding error (`sofa_io_test.go`).
...
Real/Imag/SOS via `ReadSlice` (first/last three values since E5); smoke tests via
...
- [x] **D2. MATLAB toolbox round-trip.** Document a manual procedure
...
- [x] **E3. go-hdf5 encoder/test defects found during R1c.** Not needed by
  ...
  - (2026-09-26) — fixed in
    [CWBudde/go-hdf5#5](https://github.com/CWBudde/go-hdf5/pull/5)
    (compound class bit field and member offsets, reader for compound
    versions 1–5, integer/float properties in `CreateBasicDatatypeMessage`;
    h5py reads `CreateCompoundDataset` output). go-sofa pins its commit
    `93c0ac1` until a release.
- [x] **E4. Public shape and REFERENCE_LIST access in go-hdf5.** go-sofa
  ...
  - (2026-09-26) — go-hdf5#5 adds `Dataset.Shape`/`MaxShape`/
    `NumElements`/`Datatype` and the H5DS readers (`ReferenceList`,
    `DimensionList`, `AttachedScales`, `IsDimensionScale`). go-sofa uses
    `Shape`, `Datatype` (char arrays), `ReferenceList` and
    `IsDimensionScale`; the `Info()` regexps and the hand-rolled
...
- [x] **E5. `Dataset.ReadSlice` returns zeros for 3-D+ contiguous
  ...
  - (2026-09-26) — fixed in go-hdf5#5 with regression tests (any
    start/count, also partial outer selections and 1-D stride/block).
    sofaprobe now reads only the first and last three values;
...
- [x] **E6. Root groups with many links cannot be written.** Found with
      ...
      fails with "add string to heap: local heap is full". Real SRIR and
      BRIR files with their descriptive variables hit this.
  - Acceptance: the heap and symbol table grow (or new-style link
...
- [x] **E8. String attributes are written as NC_STRING.** go-hdf5 stores
      ...
      (NC_CHAR text). The SOFA Toolbox under Octave cannot load such a
      file (D2). MATLAB untested.

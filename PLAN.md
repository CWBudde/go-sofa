...
count)` and `ReadHyperslab(sel)` (`dataset_read_hyperslab.go:65`,
    ...
    datasets are decompressed chunk by chunk without a cache (see E9).
...
- [x] **D2. MATLAB toolbox round-trip.** Document a manual procedure
  ...
  - (2026-09-26) — `scripts/matlab/roundtrip.m` (MATLAB and Octave) plus
    the Go comparer `internal/interop/toolbox`; README "Cross-validation".
    Run with GNU Octave 8.4.0 + octave-netcdf and SOFA Toolbox 2.6.0
    (`d2a83b3`): go-sofa → toolbox → go-sofa and toolbox → go-sofa are
    bit-exact for all three variables. The run found E8: SOFAload fails on
    an unmodified go-sofa file ("Error processing DatabaseName"), because
    every attribute is an NC_STRING; the documented sequence converts them
    with `scripts/matlab/char_attributes.py` first. MATLAB itself was not
    available to test.
...
- [ ] **E9. Chunk cache for hyperslab reads.** Found in C3 (go-hdf5
